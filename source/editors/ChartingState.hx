...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['FTB', "Fade to Black. Self explanatory.\nValue 1: Time in beats"],
		['Darken BG', "Darken dat BG\nONLY WORKS WITH CLASSROOM-MEXE!!\nValue 1: 1 = Darken, Anything Else = Lighten\nValue 2: Time in beats"],
		['They Appear', "Spamton NEO\nONLY WORKS WITH CLASSROOM-MEXE!!"],
		['Evil', "Sillyness cannot be contained.\nONLY WORKS WITH CLASSROOM-MEXE!!\nValue 1: Time in beats"],
		['Bad Apple', "The bad apple reference.\nValue 1: True = Darken, Anything Else = Lighten\nValue 2: Time in beats"],
		['MEXE BG', "Every day, I imagine a future where i can steal the moon\nONLY WORKS WITH CLASSROOM-MEXE!!\nValue 1: 1 = Classroom, Anything Else = No Classroom\nValue 2: Time in beats"],
	...
	**/
	...
	function addSectionUI():Void
	{
		...
		check_duetSection = new FlxUICheckBox(check_mustHitSection.x, check_mustHitSection.y + 22, null, null, "Duet section", 100);
		...
		stepperBeats = new FlxUINumericStepper(10, 80, 1, 4, 1, 6, 2);
		stepperBeats.value = getSectionBeats();
		stepperBeats.name = 'section_beats';
		blockPressWhileTypingOnStepper.push(stepperBeats);
		check_altAnim.name = 'check_altAnim';

		check_changeBPM = new FlxUICheckBox(130, 60, null, null, 'Change BPM', 100);
		check_changeBPM.checked = _song.notes[curSec].changeBPM;
		check_changeBPM.name = 'check_changeBPM';

		stepperSectionBPM = new FlxUINumericStepper(130, 80, 1, Conductor.bpm, 0, 999, 1);
		...
		sectionInputText = new FlxUIInputText(10, 115, 50, "");
		blockPressWhileTypingOn.push(sectionInputText);

		var sectButton:FlxButton = new FlxButton(65, 112, "Go to Section", function()
			{
				if (sectionInputText.text != null && sectionInputText.text != "") {
		...
		tab_group_section.add(sectButton);
	...
	function addChartingUI() {
		...
		var check_mute_pvocals = new FlxUICheckBox(check_mute_inst.x + 120, check_mute_inst.y, null, null, "Mute Player Vocals (in editor)", 100);
		...
		var	check_mute_ovocals = new FlxUICheckBox(check_mute_inst.x, check_mute_inst.y + 30, null, null, "Mute Opponent Vocals (in editor)", 100);
		...
		pVoicesVolume.value = pvocals.volume;
		...
		oVoicesVolume.value = ovocals.volume;
		...
		tab_group_chart.add(new FlxText(pVoicesVolume.x, pVoicesVolume.y - 15, 0, 'Player Volume'));
		tab_group_chart.add(new FlxText(oVoicesVolume.x, oVoicesVolume.y - 15, 0, 'Opponent Volume'));
	...
	function loadSong():Void
	{
		...
		var pfile:Dynamic;
		var ofile:Dynamic;
		pfile = Paths.pVoices(currentSongName);
		ofile = Paths.oVoices(currentSongName);
		pvocals = new FlxSound();
		if (Std.isOfType(pfile, Sound) || OpenFlAssets.exists(pfile)) {
			pvocals.loadEmbedded(pfile);
			FlxG.sound.list.add(pvocals);
		}
		ovocals = new FlxSound();
		if (Std.isOfType(ofile, Sound) || OpenFlAssets.exists(ofile)) {
			ovocals.loadEmbedded(ofile);
	...
	override function getEvent(id:String, sender:Dynamic, data:Dynamic, ?params:Array<Dynamic>)
	{
		...
		else if (id == FlxUINumericStepper.CHANGE_EVENT && (sender is FlxUINumericStepper))
		{
			...
			{
				pvocals.volume = nums.value;
			}
			else if (wname == 'ovoices_volume')
			{
				ovocals.volume = nums.value;
			}
		}
	...
	*/
	...
	function reloadGridLayer() {
		...
		for (i in 1...beatsuh) {
			var beatsep1:FlxSprite = new FlxSprite(gridBG.x, ((GRID_SIZE * (2 * curZoom)) * i) - 1).makeGraphic(Std.int(gridBG.width), 2, 0x44FF0000);
	...
	function updateGrid():Void
	{
		...
		for (i in _song.notes[curSec].sectionNotes)
		{
			...
			if(i[3] != null && note.noteType != null && note.noteType.length > 0) {
				...
				daText.setFormat(Paths.font("metro.otf"), 24, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
				daText.xAdd = -32;
				daText.yAdd = 6;
				daText.borderSize = 1;
